...
def load_images():
    backgrounds = {
        ...
        "game1": load_tif_image('img/game1/lab_one.tif', (800, 600)),
...
def game_five():
    ...
    while True:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_SPACE:
                    coins, score, last_coin_type = collect_coin(coins, player[5], score, last_coin_type)
        ...
        player[5].update(current_lab_map)
        player[5].draw(back)
...
def game_one():
    ...
    while True:
        ...
        player[1].update(current_lab_map)
        player[1].draw(back)

        draw_button('menu', width // 2 - 373, height // 2 + 59, 115, 49, (88, 130, 71), (106, 141, 94), open_menu, 255,
                    30)
        draw_circle_button('what', width // 2 - 315, height // 2 + 200, 18, (255, 255, 255), open_que, 0, 30)
        draw_button('music', width // 2 - 373, height // 2 + 118, 115, 49, (88, 130, 71), (106, 141, 94), toggle_music,
                    255, 30)
        ...
        coins, score, last_coin_type = collect_coin(coins, player[1], score, last_coin_type)
...
def game_two():
        ...
        while True:
            ...
            player[2].update(current_lab_map)
            player[2].draw(back)

            draw_button('menu', width // 2 - 373, height // 2 + 59, 115, 49, (88, 130, 71), (106, 141, 94), open_menu,
                        255, 30)
            draw_circle_button('what', width // 2 - 315, height // 2 + 200, 18, (255, 255, 255), open_que, 0)
            draw_button('music', width // 2 - 373, height // 2 + 118, 115, 49, (88, 130, 71), (106, 141, 94),
                        toggle_music, 255, 255)
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_ESCAPE:
                        return
                    if event.key == pygame.K_SPACE:
                        coins, score, last_coin_type = collect_coin(coins, player[2], score, last_coin_type)
...
def game_three():
    ...
    while True:
        ...
        player[3].update(current_lab_map)
        player[3].draw(back)
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_ESCAPE:
                    return
                if event.key == pygame.K_SPACE:
                    coins, score, last_coin_type = collect_coin(coins, player[3], score, last_coin_type)


        if not coins:
            exit_row, exit_col = 13, 15
            player_row = (player[3].rect.centery - OFFSET_Y) // CELL_SIZE
            player_col = (player[3].rect.centerx - OFFSET_X) // CELL_SIZE
            if (player_row, player_col) == (exit_row, exit_col):
...
def game_four():
    ...
    while True:
        ...
        coins, score, last_coin_type = collect_coin(coins, player[4], score, last_coin_type)

        player[4].update(current_lab_map)
        player[4].draw(back)
