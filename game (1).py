...
from lab import  lab_map, lab_map_2, lab_map_3, lab_map_4, draw_lab, player_position, goal_position, Cloud, Rain, Butterfly, Flower, find_spawn_point
from quest import check_achievements, open_achievements_back, load_achievements
...
def load_images():
    ...
    return main_menu_bg, menu_game_bg, game1_bg, game2_bg, game3_bg, game4_bg, game5_bg, game12_bg, game22_bg, game33_bg

main_menu_bg, main_game_bg, game1_bg, game2_bg, game3_bg, game4_bg, game5_bg, game12_bg, game22_bg, game33_bg = load_images()
...
player1 = Player(spawn_x, spawn_y, "img/Picsart_25-05-07_20-43-17-253.png", (40, 40), width, height)

spawn_x1, spawn_y1 = find_spawn_point(lab_map_2)
player2 = Player(spawn_x1, spawn_y1, "img/Picsart_25-05-07_20-43-17-253.png", (40, 40), width, height)

spawn_x2, spawn_y2 = find_spawn_point(lab_map_3)
player3 = Player(spawn_x2, spawn_y2, "img/Picsart_25-05-07_20-43-17-253.png", (40, 40), width, height)

spawn_x3, spawn_y3 = find_spawn_point(lab_map_4)
player4 = Player(spawn_x3, spawn_y3, "img/Picsart_25-05-07_20-43-17-253.png", (40, 40), width, height)

spawn_x4, spawn_y4 = find_spawn_point(lab_map)  # якщо вона існує
player5 = Player(spawn_x4, spawn_y4, "img/Picsart_25-05-07_20-43-17-253.png", (40, 40), width, height)
...
def start_back():
    ...
    while True:
        ...
        draw_button('', width // 2 + 149, height // 2 - 242, 220, 220, (100, 200, 100), (63, 103, 55), game_five, 12)

        draw_button("", width // 2 - 113, height // 2 + 33 , 220, 220, (100, 200, 100), (63, 103, 55), game_four, 18)
...
def game_five():
    ...
    while True:
        ...
        player5.draw(back)
        # ТУТ можна додати логіку, коли саме додавати бали:
        ...
        draw_circle_button('what', width // 2 - 315, height // 2 + 200, 18,
                           (255, 255, 255), open_que, 0)
        draw_button('music', width // 2 - 373, height // 2 + 118, 115, 49,
                    (88, 130, 71), (106, 141, 94), toggle_music, 255, 255)



        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                # Збереження результату перед виходом
...
def game_one():
    global score, current_lab_map
    current_lab_map = lab_map
    clouds = [Cloud('img/Picsart_25-05-10_13-49-48-007.png') for _ in range(25)]
...
def game_four():
    ...
    butterflies = [Butterfly('img/game4/butterfly.png') for _ in range(10)]
    while True:
...
def start2_back():
    ...
    while True:
        ...
        text_rect = text.get_rect(center=(width // 2, height // 2))
        back.blit(text, text_rect)
        draw_button('Menu', width // 2 - 250, height // 2 + 200, 300, 60, (63, 91, 120), return_to_main_menu, 100)
        draw_chat()
        color = ()
        pygame.draw.rect(back, (0, 0, 255), box, 2)
        #text = ''
        txt_surf = font9.render(str(text), True, black)
...
def settings_back():
    ...
    while True:
        ...
        draw_button('Achi', width // 2 - 190, height // 2, 300, 60, (100, 200, 100), (200, 200, 200), open_achievements_back, 10)
